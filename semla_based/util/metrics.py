...
edges_dic = {}
one_hot_dict = {"H": 0, "C": 1, "N": 2, "O": 3, "F": 4}
import numpy as np
...
class PropertyMetric(GenerativeMetric):
    def __init__(self, classifier, mean, mad, **kwargs):
        super().__init__(**kwargs)
        
        
        # To load property_name egnn
        self.classifier = classifier
        self.mean = mean
        self.mad = mad
        self.add_state("property", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("n_valid", default=torch.tensor(0), dist_reduce_fx="sum")
    def update(self, mols: List[Chem.rdchem.Mol], labels) -> None:
        

        for i, mol in enumerate(mols):
            if mol is None:
                continue
            atom_positions = torch.tensor(mol.GetConformer().GetPositions(), dtype=torch.float32, device='cuda')
            # Transform to one hot: H, C, N, O, F
            one_hot_dict = {"H": 0, "C": 1, "N": 2, "O": 3, "F": 4}
            nodes = np.vectorize(lambda x: one_hot_dict.get(x, -1))(np.array([atom.GetSymbol() for atom in mol.GetAtoms()]))
            assert -1 not in nodes, "Invalid atom type found in molecule"
            nodes = torch.tensor(nodes, dtype=torch.float32, device='cuda') # For batch
            nodes = torch.nn.functional.one_hot(nodes.to(torch.int64), num_classes=5).float()
            # Get atom mask
            atom_mask = torch.ones((nodes.shape[0], 1), dtype=torch.bool, device='cuda')
            edge_mask = torch.ones((nodes.shape[0], nodes.shape[0]), dtype=torch.float32, device='cuda')
            # Remove diagonal
            diag_mask = ~torch.eye(edge_mask.size(1), dtype=torch.bool).unsqueeze(0)
            diag_mask = diag_mask.to('cuda')
            edge_mask *= diag_mask.squeeze(dim=0)
            edge_mask = edge_mask.view(nodes.shape[0] * nodes.shape[0], 1)
            n_nodes = nodes.shape[0]
            edges = get_adj_matrix(n_nodes, 1, 'cuda')
            pred = self.classifier(h0=nodes, x=atom_positions, edges=edges, edge_attr=None, node_mask=atom_mask, edge_mask=edge_mask,
                        n_nodes=n_nodes)
            
            # l1 loss
            self.property += (torch.nn.functional.l1_loss(pred * self.mad + self.mean, labels[i].to('cuda')) ).sum().item()
            self.n_valid += 1

    def compute(self) -> torch.Tensor:
        return self.property / self.n_valid
