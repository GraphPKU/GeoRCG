...
class GeometricMol(SmolMol):
    ...
    @staticmethod
    def from_rdkit_property_pair(mol: Chem.rdchem.Mol, property: _T, property_name: str) -> GeometricMol:
        ...
        mol = GeometricMol(coords, atomics, bond_indices, bond_types, charges=charges, str_id=smiles, property=property, property_name=property_name)
        return mol
    ...
    def _copy_with(
        ...
        property = self.property if property is None else property
        property_name = self.property_name if property_name is None else property_name
...
class GeometricMolBatch(SmolBatch[GeometricMol]):
    ...
    # *** Geometric Specific Properties ***
    @property
    def properties(self) -> _T:
        if self._properties is None:
            properties = [mol.property for mol in self._mols]
            if None not in properties:
                self._properties = torch.tensor(properties)
            elif (properties[0] is None and None not in properties[1:]):
                self._properties = [-1.0] + properties[1:]
                self._properties = torch.tensor(self._properties)
