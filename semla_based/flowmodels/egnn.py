...
class VanillaEgnnLayer(torch.nn.Module):
    def __init__(self, d_model, in_edge_feats, d_pred_edge=None, norm=False, eps=1e-5,
                 d_rep=None, n_cross_attn_heads=4, dropout=0.0, attn_block_num=1):
        super().__init__()
    ...
    def forward(self, coords, inv_feats, adj_matrix, atom_mask, edge_feats, rep=None):
        ...
        """
        # TODO: finish cross attention
        if rep is not None:
            assert self.d_rep is not None
            for layer in self.attn:
                inv_feats = layer(inv_feats, context=rep.unsqueeze(1)) * atom_mask
...
class VanillaEgnnDynamics(torch.nn.Module):
    def __init__(self, d_model, n_layers, d_edge, d_rep=None):
        super().__init__()
        ...
        in_edge_feats = 1
        layers = [VanillaEgnnLayer(d_model, in_edge_feats, norm=True, d_rep=d_rep) for _ in range(n_layers - 2)]
    ...
    def forward(self, coords, inv_feats, adj_matrix, atom_mask=None, edge_feats=None, rep=None):
        ...
        for layer in self.layers:
            ...
            coords, inv_feats = layer(coords, inv_feats, adj_matrix, atom_mask, dist_feats, rep=rep)
...
class VanillaEgnnGenerator(MolecularGenerator):
    def __init__(
        ...
        self.dynamics = VanillaEgnnDynamics(d_model, n_layers, d_edge, d_rep=d_rep)
