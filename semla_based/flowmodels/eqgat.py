...
class EqgatLayer(torch.nn.Module):
    def __init__(self, d_model, n_equi_feats, d_edge, eps=1e-5,
        ...
        if d_rep is not None:
            ...
            # TODO: finish transformer block

    def forward(self, coords, inv_feats, equi_feats, adj_matrix, atom_mask, edge_feats, rep=None):
        """Pass data through one layer of the model
        ...
        """
        # TODO: implement representation condition
        if rep is not None:
...
class EqgatDynamics(torch.nn.Module):
    def __init__(self, d_model, n_layers, n_equi_feats, d_edge, eps=1e-5, d_rep=None):
        super().__init__()

        layers = [EqgatLayer(d_model, n_equi_feats, d_edge, eps=eps, d_rep=d_rep) for _ in range(n_layers)]
...
class EqgatGenerator(MolecularGenerator):
    def __init__(
        ...
        self.dynamics = EqgatDynamics(d_model, n_layers, n_equi_feats, d_edge, d_rep=d_rep)
