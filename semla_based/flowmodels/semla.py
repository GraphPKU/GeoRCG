...
class CoordNorm(torch.nn.Module):
    ...
    Note that 'length' provides the same coordinate normalisation that is commonly used in current flowmodels but adapted
...
class EquiMessagePassingLayer(torch.nn.Module):
    ...
    def forward(self, coords, node_feats, adj_matrix, node_mask, edge_feats=None, rep=None):
        """Pass data through the layer
...
class EquiInvDynamics(torch.nn.Module):
    ...
    def forward(self, coords, inv_feats, adj_matrix, atom_mask=None, edge_feats=None, cond_coords=None, rep=None, times=None):
        ...
        for layer in self.layers:
            out = layer(coords, inv_feats, adj_matrix, atom_mask, edge_feats=edge_feats, rep=rep)
            if len(out) == 2:
...
class SemlaGenerator(MolecularGenerator):
    def __init__(
        ...
        in_feats = in_feats + property_emb if property_condition else in_feats
        ...
        if self.property_condition:
            self.property_proj = torch.nn.Sequential(
                torch.nn.Linear(1, property_emb),
                torch.nn.SiLU(inplace=False),
    ...
    def forward(
        ...
        if self.property_condition:
            assert property is not None, "property is None in SemlaGenerator but property_condition=True"
        ...
        if self.property_condition:
            # property: [batch_size, 1]
            property = self.property_proj(property.unsqueeze(-1))
            property = property.unsqueeze(1).repeat(1, inv_feats.size(1), 1)
            inv_feats = torch.cat((inv_feats, property), dim=-1)
