...
class MolecularCFM_property(L.LightningModule):
    def __init__(
        self,
        ...
        **kwargs
        ...
        # self.property_metric = Metrics.PropertyMetric(classifier=classifier, mean=property_norms["mean"], mad=property_norms["mad"])
        ...
        self.pre_generated_reps = []
        self.pre_generated_reps_sizes = []
        self.pre_generated_reps_properties = []
    ...
    def forward(self, batch, t, training=False, cond_batch=None, rep=None):
        ...
        if training:
            ...
            return self._forward(batch, t, training=training, cond_batch=cond_batch, rep=rep, properties=properties)
        else:
            ...
            for key in batch.keys():
                augumented_batch[key] = torch.cat([batch[key], batch[key]], dim=0)
            ...
            if cond_batch is not None:
                ...
                output = self._forward(augumented_batch, augumented_t, training=training, cond_batch=augumented_cond_batch, rep=augumented_rep, properties=properties)
            else:
                output = self._forward(augumented_batch, augumented_t, training=training, rep=augumented_rep, properties=properties)
    ...
    def _forward(self, batch, t, training=False, cond_batch=None, rep=None, properties=None):
        ...
        Args:
            ...
            rep (torch.Tensor): sampled representation, shape [batch_size, rep_dim]
    ...
    def validation_step(self, batch, b_idx):
        ...
        gen_batch = self._generate_uncond(prior, self.integrator.steps, self.sampling_strategy)
        ...
        property_labels = prior["properties"]
        # self.property_metric.update(gen_mols, property_labels)
        ...
        if self.pairwise_metrics:
            ...
            gen_interp_batch = self._generate_uncond(interpolated, gen_interp_steps)
    ...
    def on_validation_epoch_end(self):
        ...
        # property_metrics_results = self.property_metric.compute()
        metrics = {
            **stability_metrics_results,
            **gen_metrics_results,
            **pair_metrics_results,
            # "property": property_metrics_results
        }
    ...
    def _generate_reps_conditional(self, lengths, steps, strategy="linear", properties=None):

        with torch.no_grad():
            s = time.time()
            device = lengths.device
            batch_size = lengths.shape[0]
            nodesxsample = lengths
            ...
            properties_condition = (properties - self.property_norms["mean"]) / self.property_norms["mad"]
            
            sampled_rep = self.rdm.sample(
                nodesxsample=nodesxsample,
                device=device,
                additional_cond=properties_condition,
                running_batch_size=batch_size
            )
            print(f"Rep sampling time for {batch_size} samples: {time.time() - s}")

            
            self.pre_generated_reps.append(sampled_rep)
            self.pre_generated_reps_sizes.append(nodesxsample)
            property_to_append = properties_condition * self.property_norms["mad"] + self.property_norms["mean"]
            self.pre_generated_reps_properties.append(property_to_append)
            
            
    def _generate_uncond(self, prior, steps, strategy="linear"):
        if self.distill:
        ...
        with torch.no_grad():    
            ...
            if len(self.pre_generated_reps) > 0:
                ...
                for size in nodesxsample:
                    ...
                    pre_generated_properties = self.pre_generated_reps_properties[mask][rand_idx]
                    sampled_rep.append(pre_generated_reps)
                    sampled_rep_properties.append(pre_generated_properties)
                sampled_rep = torch.stack(sampled_rep, dim=0)
                sampled_rep_properties = torch.stack(sampled_rep_properties, dim=0).squeeze(-1)
                # NOTE: Overwrite the properties in the prior
            ...
            for step_size in step_sizes:
                ...
                coords, type_logits, bond_logits, charge_logits = self(curr, times, training=False, cond_batch=cond, rep=sampled_rep)
        ...
        print(f"Integration time for {batch_size} samples: {time.time() - s}")
        return predicted

    def _generate(self, prior, steps, strategy="linear", prop_sampler=None):
        if self.distill:
            return self._distill_generate(prior)
        ...
        with torch.no_grad():    
            ...
            properties = prop_sampler.sample_batch(nodesxsample).squeeze(-1)
            # NOTE: Overwrite the properties in the prior
            curr["properties"] = properties.to(device)
            ...
            if len(self.pre_generated_reps) > 0:
                ...
                sampled_rep_properties = torch.stack(sampled_rep_properties, dim=0).squeeze(-1)
                # NOTE: Overwrite the properties in the prior
                curr["properties"] = sampled_rep_properties.to(device)
        ...
        predicted["coords"] = predicted["coords"] * self.coord_scale
        print(f"Integration time for {batch_size} samples: {time.time() - s}")
        return predicted, curr["properties"]
