...
class MolecularCFM(L.LightningModule):
    ...
    def forward(self, batch, t, training=False, cond_batch=None, rep=None):
        ...
        if training:
            ...
            if cond_batch is not None:
                ...
                output = self._forward(augumented_batch, augumented_t, training=training,
                                       cond_batch=augumented_cond_batch, rep=augumented_rep)
            else:
    ...
    def _forward(self, batch, t, training=False, cond_batch=None, rep=None):
        ...
        if cond_batch is not None:
            ...
            out = model(coords, features, edge_feats=bonds, atom_mask=mask, rep=rep, times=t if self.time_condition else None)
    ...
    def training_step(self, batch, b_idx):
        ...
        rep = get_global_representation(mask, self.encoder, coords, atom_types, training_encoder=False,
                                        noise_sigma=noise_sigma, dataset=self.dataset)
        assert (rep.requires_grad and False) or (not rep.requires_grad and not False)

        clean_rep = get_global_representation(mask, self.encoder, coords, atom_types, training_encoder=False,
                                              noise_sigma=0., dataset=self.dataset)
        ...
        predicted_rep = get_global_representation(predicted_mask, self.encoder, predicted_coords, predicted_atom_types,
                                                  training_encoder=False, noise_sigma=0., dataset=self.dataset)
