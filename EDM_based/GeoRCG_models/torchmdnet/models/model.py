...
from GeoRCG_models.torchmdnet.models import output_modules
from GeoRCG_models.torchmdnet.models.wrappers import AtomFilter
from GeoRCG_models.torchmdnet import priors
import warnings


def create_model(args, prior_model=None, mean=None, std=None, only_representation=True):
    shared_args = dict(
    ...
    if args["encoder_model"] == "graph-network":
        from GeoRCG_models.torchmdnet.models.torchmd_gn import TorchMD_GN

        is_equivariant = False
        representation_model = TorchMD_GN(
            num_filters=args["embedding_dimension"], aggr=args["aggr"], **shared_args
        )
    elif args["encoder_model"] == "transformer":
        from GeoRCG_models.torchmdnet.models.torchmd_t import TorchMD_T
    ...
    elif args["encoder_model"] == "equivariant-transformer":
        from GeoRCG_models.torchmdnet.models.torchmd_et import TorchMD_ET
    ...
    if only_representation:
        class fake_model(nn.Module):
            ...
            def forward(self, z, pos, batch):
                return self.representation_model(z, pos, batch)
        
        
        return fake_model(representation_model)
...
def load_model(filepath, args, device="cpu", mean=None, std=None, only_representation=True, **kwargs):
    
    model = create_model(args, only_representation=only_representation)
    
    if filepath == "" or filepath is None:
        print("Warning: You are not using any pretrained weight, therefore random representations!")
    else:
    ...
    model.mean = None # representation model does not use these statistic data.
    model.std = None
