...
@register_model("unimol_global")
class UniMolModelGlobal(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
    ...
    def __init__(self, args, dictionary):
        ...
        self.node_feature_fusion = nn.Sequential(
            nn.Linear(args.encoder_embed_dim, args.encoder_embed_dim),
            nn.GELU(),
            nn.Linear(args.encoder_embed_dim, args.encoder_embed_dim),
            nn.GELU(),
    ...
    def forward(
        ...
        clean_global_rep=None,
        output_global_rep_only=False,
        **kwargs
    ):
        ...
        # NOTE: This is for global representation intervention
        if clean_global_rep is not None:
            x = x + self.node_feature_fusion(clean_global_rep).unsqueeze(1)
...
@register_model_architecture("unimol_global", "unimol_global")
