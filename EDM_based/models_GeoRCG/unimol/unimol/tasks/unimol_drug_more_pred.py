...
class ComposedDataset:
    def __init__(self, dataset_list):
        ...
        # self.preprocess_energy()
        
    def preprocess_energy(self):
        from tqdm import tqdm
        for idx in tqdm(range(len(self)), desc="Preprocessing energy"):
            if idx < 1000:
                data = self[idx]
                data["energy"] = self.energy_calculator(data)
            else:
                data["energy"] = None
            a = 1
                
    
    def energy_calculator(self, data):
        ...
        energy = smolRD.calc_energy(rdkit_mol, per_atom=False) if rdkit_mol is not None else None
        def _is_valid_float(num):
            return num not in [None, float("inf"), float("-inf"), float("nan")]
        valid_energy = energy if _is_valid_float(energy) else None
        if valid_energy is None:
            print(f"Invalid energy for {data['smi']}")
            valid_energy = - 1e6
        return valid_energy
    ...
    def __getitem__(self, idx):
        for dataset in self.dataset_list:
            if idx < len(dataset):
                return_item = dataset[idx]
                return_item["energy"] = self.energy_calculator(return_item)
                return return_item
            else:
                idx -= len(dataset)
        raise IndexError
...
@register_task("unimol_drug_more_pred")
class UniMolDrugMorePredTask(UnicoreTask):
    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        def one_dataset(raw_dataset, coord_seed, mask_seed):
            ...
            energy_dataset = KeyDataset(raw_dataset, "energy")
            ...
            # Add clean target
            encoder_target_dataset_full = KeyDataset(expand_dataset, "targets_full")
            expand_dataset[0]
            tgt_dataset_full= PrependAndAppend(
                encoder_target_dataset_full, self.dictionary.bos(), self.dictionary.bos()
            )
            target_edge_type = EdgeTypeDataset(tgt_dataset_full, len(self.dictionary))
            ...
            return {
                ...
                "energy_target": RawArrayDataset(energy_dataset),
                "tokens_target_full": RightPadDataset(
                    tgt_dataset_full,
                    pad_idx=self.dictionary.pad(),
                ),
                "edge_type_target": RightPadDataset2D(
                    target_edge_type,
                    pad_idx=0,
                ),
            }
