...
def instantiate_from_multiple_config(config):
    ...
    for i in range(cond_num):
        ...
        params_name = f"params{idx}"
        models.append(get_obj_from_str(config[target_name])(**config.get(params_name, dict())))
    
    assert len(models) == cond_num
    return models
