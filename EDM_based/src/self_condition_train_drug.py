...
import hydra           
@hydra.main(config_path="../configs", config_name="drug_pcdm_config.yaml", version_base="1.3")
def main(args):
    ...
    if pcdm_args.debug:
        ...
        pcdm_args.n_stability_samples = 4
    ...
    if not getattr(pcdm_args, "midi_geom_data", False):
        dataloaders, charge_scale = dataset.retrieve_dataloaders(pcdm_args)
    else:
        dataloaders, charge_scale = dataset.retrieve_dataloaders_midi_geom(pcdm_args)
    ...
    # Set up the sampler
    rep_sampler = initilize_rep_sampler(pcdm_args, device, pcdm_args, debug=pcdm_args.debug)
    ...
    assert pcdm_args.dataset != 'geom' or (not pcdm_args.include_charges), "We do not use charge features for geom dataset."
    ...
    for epoch in range(pcdm_args.start_epoch, pcdm_args.n_epochs):
        ...
        if epoch % pcdm_args.test_epochs == 0 or santi_check_about_sampling:
            ...
            if not pcdm_args.break_train_epoch and rank == 0 and not pcdm_args.finetune_encoder and len(pcdm_args.conditioning) == 0:
                analyze_and_save(args=pcdm_args, epoch=epoch, model_sample=sampler, nodes_dist=nodes_dist,
                                 dataset_info=dataset_info, device=device,
                                 prop_dist=prop_dist, n_samples=pcdm_args.n_stability_samples, batch_size=50)
            if pcdm_args.dp: dist.barrier()
            nll_epoch_val, n_samples_val = test(args=pcdm_args, loader=dataloaders['val'], epoch=epoch, eval_model=model_ema_dp,
