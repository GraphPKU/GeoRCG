...
def load_smiles(dataset='QM9', load_train=True, test_dataset_indices=None):
    if dataset == 'QM9':
        col = 'SMILES1'
    ...
    if test_dataset_indices is not None: # double check
        assert len(test_idx) == len(test_dataset_indices)
        assert set(test_idx) == set(test_dataset_indices)
    ...
    if load_train:
        ...
        return list(df[col].loc[test_idx])
