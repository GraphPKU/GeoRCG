...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=3, attention=False,
        ...
        if self.cross_attn:
            ...
            # if self.rep_nf != self.hidden_nf: # Handle GEOM case. TODO: Use dataset name to identify it.
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None, rep=None):
        ...
        for i in range(0, self.n_layers):
            if self.cross_attn:
                ...
                # else:
                #     rep_proj = rep
                h = self.attns[i](h.view(batch_size, max_node_num, self.hidden_nf), context=rep.unsqueeze(1)).flatten(0, 1)
                h = h * node_mask
                # if self.rep_nf != self.hidden_nf:
