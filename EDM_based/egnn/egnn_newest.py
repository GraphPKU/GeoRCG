...
class EGNN(nn.Module):
    def __init__(self, in_node_nf, in_edge_nf, hidden_nf, device='cpu', act_fn=nn.SiLU(), n_layers=3, attention=False,
                 ...
                 sin_embedding=False, normalization_factor=100, aggregation_method='sum', rep_nf=None, attn_dropout=None, attn_block_num=1):
    ...
    def forward(self, h, x, edge_index, node_mask=None, edge_mask=None, rep=None):
        ...
        for i in range(0, self.n_layers):
            for block in self.attns[i]:
                h = block(h.view(batch_size, max_node_num, self.hidden_nf), context=rep.unsqueeze(1)).flatten(0, 1)
                h = h * node_mask
