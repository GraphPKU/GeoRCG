...
class DistributionProperty_Semla:
    def __init__(self, dataloader, properties, num_bins=1000, normalizer=None):
        self.num_bins = num_bins
        self.distributions = {}
        self.properties = properties
        assert len(properties) == 1, "Only one property is supported for now"
        assert dataloader.dataset[0].property_name == properties[0], f"{dataloader.dataset[0].property_name} != {properties}"
        for prop in properties:
            self.distributions[prop] = {}
            self._create_prob_dist(torch.tensor(dataloader.dataset.lengths, dtype=torch.int64),
                                   dataloader.dataset._data.properties,
                                   self.distributions[prop])
    ...
    def sample(self, n_nodes=19):
        ...
        for prop in self.properties:
            ...
            # val = self.normalize_tensor(val, prop) # Normalize is done within the SemlaFlow model
            vals.append(val)
        vals = torch.cat(vals)
        return vals
