...
@register_loss("unimol_pred")
class UniMolPredLoss(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # This feeds clean structure to the model to get the energy prediction
        clean_masked_tokens = torch.zeros_like(masked_tokens).bool()
        (
            _,
            _,
            _,
            ...
            src_tokens=sample[target_key]["tokens_target_full"],
            src_coord=sample[target_key]["coord_target"],
            src_distance=sample[target_key]["distance_target"],
            src_edge_type=sample[target_key]["edge_type_target"],
            encoder_masked_tokens=clean_masked_tokens)
        
        # For energy prediction
        energy_pred = energy_pred.squeeze()
        energy_target = sample[target_key]["energy_target"].to(energy_pred.dtype)
        
        valid_energy_prediction_mask = energy_target > -1e4 
        energy_loss = F.mse_loss(
            energy_pred[valid_energy_prediction_mask],
            energy_target[valid_energy_prediction_mask],
            reduction="mean",
        )
