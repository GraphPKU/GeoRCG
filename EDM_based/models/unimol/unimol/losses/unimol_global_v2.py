...
@register_loss("unimol_global_v2")
class UniMolLossGlobal_V2(UnicoreLoss):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        (
            ...
            _,
            input_global_rep,
        ) = model(**sample[clean_input_key], encoder_masked_tokens=None)
        ...
        ) = model(**sample[difficult_input_key], encoder_masked_tokens=difficult_masked_tokens, use_lightweight_model=True, input_global_rep=input_global_rep)
...
@register_loss("unimol_infer_global_v2")
class UniMolInferGlobalLoss_V2(UnicoreLoss):
    def __init__(self, task):
        super().__init__(task)
        self.padding_idx = task.dictionary.pad()
