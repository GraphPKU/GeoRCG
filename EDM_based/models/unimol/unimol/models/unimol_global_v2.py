...
@register_model("unimol_global_v2")
class UniMolModelGlobal_V2(BaseUnicoreModel):
    @staticmethod
    def add_args(parser):
        ...
        parser.add_argument(
            "--difficult-masked-token-loss",
            type=float,
            metavar="D",
            help="mask loss ratio",
        )
        parser.add_argument(
            "--difficult-masked-dist-loss",
            type=float,
            metavar="D",
            help="masked distance loss ratio",
        )
        parser.add_argument(
            "--difficult-masked-coord-loss",
            type=float,
            metavar="D",
            help="masked coord loss ratio",
        )
        parser.add_argument(
            "--difficult-x-norm-loss",
            type=float,
            metavar="D",
            help="x norm loss ratio",
        )
        parser.add_argument(
            "--difficult-delta-pair-repr-norm-loss",
            type=float,
            metavar="D",
            help="delta encoder pair repr norm loss ratio",
        )
        parser.add_argument(
            "--difficult-masked-coord-dist-loss",
            type=float,
            metavar="D",
            help="masked coord dist loss ratio",
        )
    ...
    def __init__(self, args, dictionary):
        ...
        # NOTE: This is for global representation summarization
        self.global_rep_head = nn.Sequential(
        ...
        # NOTE: Lightweight model that is not used but for the sake of proxy task
        lightweight_args = copy.deepcopy(args)
        lightweight_args.encoder_layers = lightweight_args.encoder_layers // 2
        print(f"Setting lightweight model with {lightweight_args.encoder_layers} layers")
        
        args.num_layers = None # Lightweight model should use all layers
        args.first4 = None # Lightweight model should use all layers
        self.lightweight_model = UniMolModel(lightweight_args, dictionary, global_condition=True)
    ...
    def forward(
        ...
        input_global_rep=None,
        **kwargs
    ):
        assert use_lightweight_model == (input_global_rep is not None), "use_lightweight_model and input_global_rep should be the same"
        if use_lightweight_model:
            return self.lightweight_model(
                ...
                encoder_masked_tokens=encoder_masked_tokens,
                features_only=features_only,
                classification_head_name=classification_head_name,
                input_global_rep=input_global_rep,
                **kwargs
            )
        ...
        # NOTE: graph-level prediction by mean pooling
        graph_rep = encoder_rep.mean(dim=1)
        graph_rep = self.global_rep_head(graph_rep)
        # NOTE: Additional layer normalization without learnable parameters
        graph_rep_mean = graph_rep.mean(dim=-1, keepdim=True)
        graph_rep_std = graph_rep.std(dim=-1, keepdim=True)
        graph_rep = (graph_rep - graph_rep_mean) / (graph_rep_std + 1e-6)
        
        
        if kwargs.get("output_global_rep_only", False):
            return graph_rep
        ...
        if self.args.mode == 'infer':
            return encoder_rep, encoder_pair_rep, graph_rep
        else:
            return (
                logits,
                encoder_distance,
...
@register_model_architecture("unimol_global_v2", "unimol_global_v2")
def unimol_global_v2_architecture(args):
