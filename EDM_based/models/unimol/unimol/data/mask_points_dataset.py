...
class MaskPointsDataset(BaseWrapperDataset):
    ...
    def __getitem_cached__(self, epoch: int, index: int):
        ...
        with data_utils.numpy_seed(self.seed, epoch, index):
            ...
            # NOTE: This is the tokens of clean structure
            ret["targets_full"] = np.full(len(mask), self.pad_idx)
            clean_mask = np.ones_like(mask).astype(bool)
            ret["targets_full"][clean_mask] = item[clean_mask]
            ret["targets_full"] = torch.from_numpy(ret["targets_full"]).long()
